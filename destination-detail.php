...
$galleryImages = !empty($d['gallery']) ? (json_decode($d['gallery'], true) ?: []) : [];
$showBestTime = !isset($d['show_best_time']) || (int)$d['show_best_time'] === 1;
...
<head>
  ...
  <?php
  $ogImg  = $d['hero_image']
      ? (strpos($d['hero_image'], 'uploads/') === 0 ? siteUrl($d['hero_image']) : siteUrl('images/' . $d['hero_image']))
      : seoImageUrl('images/dest-arugam.jpg');
          ...
          <div class="dest-gallery-grid">
            <?php foreach ($galleryImages as $galleryImage): ?>
              <div class="dest-gallery-item">
                <img src="<?= e(destImgD($galleryImage)) ?>" alt="<?= e($d['title']) ?> gallery image"/>
              </div>
            <?php endforeach; ?>
          </div>
        </div>
