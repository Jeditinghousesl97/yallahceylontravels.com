...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['booking_submit'])) {
    ...
    if ($turnstileSecretKey && !empty($_POST['cf-turnstile-response'])) {
        $verify = @file_get_contents('https://challenges.cloudflare.com/turnstile/v0/siteverify', false,
            ...
            'content' => http_build_query(['secret' => $turnstileSecretKey, 'response' => $_POST['cf-turnstile-response']])]]));
        $res = $verify ? json_decode($verify, true) : null;
        if (!$res || !$res['success']) $bookingErrors[] = 'Security check failed. Please try again.';
    } elseif ($turnstileSecretKey) {
        $bookingErrors[] = 'Please complete the security check.';
    }
    ...
    if (!$bookingErrors && $conn) {
        ...
        if ($stmt->execute()) {
            ...
            if ($recipient !== '') {
                $subject = 'New Tour Booking - ' . $bTitle;
                $htmlBody = '
                    <h2 style="margin:0 0 16px;">New Tour Booking</h2>
                    <table cellpadding="8" cellspacing="0" border="0" style="border-collapse:collapse;width:100%;max-width:700px;">
                        <tr><td style="width:180px;"><strong>Tour</strong></td><td>' . e($bTitle) . '</td></tr>
                        <tr><td><strong>Name</strong></td><td>' . e($bName) . '</td></tr>
                        <tr><td><strong>Email</strong></td><td>' . e($bEmail) . '</td></tr>
                        <tr><td><strong>Phone</strong></td><td>' . e($bPhone !== '' ? $bPhone : '-') . '</td></tr>
                        <tr><td><strong>Tour Date</strong></td><td>' . e($bDate) . '</td></tr>
                        <tr><td><strong>Persons</strong></td><td>' . e($bPersons !== '' ? $bPersons : '-') . '</td></tr>
                        <tr><td><strong>IP Address</strong></td><td>' . e($ip !== '' ? $ip : '-') . '</td></tr>
                        <tr><td valign="top"><strong>Special Requests</strong></td><td>' . nl2br(e($bMsg !== '' ? $bMsg : '-')) . '</td></tr>
                    </table>
                ';
                $textBody = "New Tour Booking\n\n"
                    . "Tour: {$bTitle}\n"
                    . "Name: {$bName}\n"
                    . "Email: {$bEmail}\n"
                    . 'Phone: ' . ($bPhone !== '' ? $bPhone : '-') . "\n"
                    . "Tour Date: {$bDate}\n"
                    . 'Persons: ' . ($bPersons !== '' ? $bPersons : '-') . "\n"
                    . 'IP Address: ' . ($ip !== '' ? $ip : '-') . "\n\n"
                    . "Special Requests:\n" . ($bMsg !== '' ? $bMsg : '-') . "\n";

                sendSiteEmail($recipient, $subject, $htmlBody, $textBody, [
                    'reply_to_email' => $bEmail,
                    'reply_to_name' => $bName,
                ]);
            }
...
<head>
  ...
  <?php
  $ogImg = $t['image'] ? imgUrl($t['image']) : seoImageUrl('images/tour-kandy.jpg');
  ...
  <link rel="stylesheet" href="css/tour-detail.css?v=<?= file_exists(__DIR__ . '/css/tour-detail.css') ? filemtime(__DIR__ . '/css/tour-detail.css') : time() ?>"/>
  ...
  <div class="container">
    <div class="detail-grid">
      ...
      <div class="detail-main">
              ...
              <?php foreach ($itinItems as $di => $day): ?>
                <div class="itinerary-item">
                  ...
                  <div class="itin-content">
                    ...
                    <?php if ($day['body']): ?><p><?= nl2br(e($day['body'])) ?></p><?php endif; ?>
                    <?php if (!empty($day['image1']) || !empty($day['image2'])): ?>
                      <div class="itin-photos">
                        <?php if (!empty($day['image1'])): ?><img src="<?= e(imgUrl($day['image1'])) ?>" alt="<?= e($day['title']) ?>" loading="lazy"/><?php endif; ?>
                        <?php if (!empty($day['image2'])): ?><img src="<?= e(imgUrl($day['image2'])) ?>" alt="<?= e($day['title']) ?>" loading="lazy"/><?php endif; ?>
                      </div>
                    <?php endif; ?>
                  </div>
                </div>
        ...
        <!-- INCLUDES / EXCLUDES -->
        <?php if ($includes || $excludes || !empty(trim(strip_tags($t['includes'] ?? ''))) || !empty(trim(strip_tags($t['excludes'] ?? '')))): ?>
          <div class="detail-section inc-exc-section reveal">
            <h2 class="detail-section-title"><i class="fas fa-clipboard-list"></i> What's Included &amp; Excluded</h2>
            <div class="inc-exc-grid">
              <?php if ($includes || !empty(trim(strip_tags($t['includes'] ?? '')))): ?>
              <div class="inc-col">
                ...
                <?php if ($includes): ?>
                  ...
                  <?php endforeach; ?>
                <?php else: ?>
                  <div class="rich-content"><?= $t['includes'] ?></div>
                <?php endif; ?>
              </div>
              <?php endif; ?>
              <?php if ($excludes || !empty(trim(strip_tags($t['excludes'] ?? '')))): ?>
              <div class="exc-col">
                ...
                <?php if ($excludes): ?>
                  ...
                  <?php endforeach; ?>
                <?php else: ?>
                  <div class="rich-content"><?= $t['excludes'] ?></div>
                <?php endif; ?>
              </div>
              <?php endif; ?>
            </div>
      ...
      <div class="detail-sidebar">
        ...
        <div class="booking-card" id="booking">
          ...
          <div class="booking-card-body">
            ...
            <form method="POST" action="<?= e($_SERVER['REQUEST_URI']) ?>#booking">
              ...
              <?php if ($turnstileSiteKey): ?>
                <div class="cf-turnstile" data-sitekey="<?= e($turnstileSiteKey) ?>" style="margin:0 0 14px"></div>
              <?php endif; ?>
        ...
        <!-- Journey Highlights -->
        <?php if ($highlights || !empty(trim(strip_tags($t['highlights'] ?? '')))): ?>
          <div class="highlights-card reveal">
            ...
            <div class="highlight-list">
              <?php if ($highlights): ?>
                ...
                <?php endforeach; ?>
              <?php else: ?>
                <div class="rich-content"><?= $t['highlights'] ?></div>
              <?php endif; ?>
            </div>
          </div>
        <?php endif; ?>
...
<script src="js/animations.js?v=<?= file_exists(__DIR__ . '/js/animations.js') ? filemtime(__DIR__ . '/js/animations.js') : time() ?>"></script>
<script>
