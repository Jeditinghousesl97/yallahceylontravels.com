...
register_shutdown_function(function () {
    ...
    if ($error && in_array($error['type'], [E_ERROR, E_PARSE, E_CORE_ERROR, E_COMPILE_ERROR, E_USER_ERROR], true)) {
        adminLogError('admin_tours_fatal', [
            'type'    => $error['type'],
...
function tourColumnExists($conn, $table, $column) {
    $tableE  = $conn->real_escape_string($table);
    $columnE = $conn->real_escape_string($column);
    $res = $conn->query("SHOW COLUMNS FROM `$tableE` LIKE '$columnE'");
    return $res && $res->num_rows > 0;
}

function ensureToursSchema($conn) {
    $changes = [
        "ALTER TABLE `tours` ADD COLUMN `tour_type` VARCHAR(120) DEFAULT NULL AFTER `duration`",
        "ALTER TABLE `tours` ADD COLUMN `faqs` LONGTEXT DEFAULT NULL AFTER `excludes`",
        "ALTER TABLE `tours` ADD COLUMN `seo_data` LONGTEXT DEFAULT NULL AFTER `map_embed`",
    ];
    $required = ['tour_type', 'faqs', 'seo_data'];

    foreach ($required as $i => $column) {
        if (!tourColumnExists($conn, 'tours', $column)) {
            if (!$conn->query($changes[$i])) {
                adminLogError('admin_tours_schema', [
                    'column' => $column,
                    'mysql'  => $conn->error,
                ]);
                return "Tours schema update failed for `$column`: " . $conn->error;
            }
        }
    }
    return '';
...
function itineraryUploadFile($files, $index) {
    if (
        !is_array($files) ||
        empty($files['name'][$index]) ||
        empty($files['tmp_name'][$index]) ||
...
$schemaError = ensureToursSchema($conn);
if ($schemaError) {
    $formError = $schemaError;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$formError) {
    ...
    if ($itinerary !== '') {
        $decodedItinerary = json_decode($itinerary, true);
        if (is_array($decodedItinerary)) {
            $existingImage1 = $_POST['itin_existing_image1'] ?? [];
            $existingImage2 = $_POST['itin_existing_image2'] ?? [];
            $builtItinerary = [];

            foreach ($decodedItinerary as $i => $day) {
                $dayTitle = trim($day['title'] ?? '');
                $dayBody  = trim($day['body'] ?? '');
                if ($dayTitle === '') continue;

                $img1 = trim($existingImage1[$i] ?? ($day['image1'] ?? ''));
                $img2 = trim($existingImage2[$i] ?? ($day['image2'] ?? ''));

                $upload1 = itineraryUploadFile($_FILES['itin_image1'] ?? [], $i);
                if ($upload1) {
                    $up1 = uploadImage($upload1, 'uploads/tours/itinerary');
                    if ($up1['ok']) {
                        $img1 = $up1['path'];
                    } else {
                        $formError = $up1['msg'];
                ...
                $upload2 = itineraryUploadFile($_FILES['itin_image2'] ?? [], $i);
                if ($upload2) {
                    $up2 = uploadImage($upload2, 'uploads/tours/itinerary');
                    if ($up2['ok']) {
                        $img2 = $up2['path'];
                    } else {
                        $formError = $up2['msg'];
                ...
                $dayRow = ['title' => $dayTitle, 'body' => $dayBody];
                if ($img1 !== '') $dayRow['image1'] = $img1;
                if ($img2 !== '') $dayRow['image2'] = $img2;
                $builtItinerary[] = $dayRow;
            }

            if (!$formError) {
                $itinerary = $builtItinerary ? json_encode($builtItinerary, JSON_UNESCAPED_UNICODE) : '';
            }
        }
    }
    ...
    if (!$title) {
        ...
        if (!empty($_FILES['image']['name'])) {
            ...
            if ($up['ok'])  $imagePath = $up['path'];
                ...
                $formError = $up['msg'];
                adminLogError('admin_tours_upload', [
                    'message' => $formError,
        ...
        if (!$formError) {
            ...
            if ($postId) {
                $ok = $conn->query("UPDATE tours SET
                ...
                $formError = 'Unable to update tour: ' . $conn->error;
                adminLogError('admin_tours_update', [
                    'mysql'   => $conn->error,
                    'tour_id' => $postId,
                    'title'   => $title,
                    'slug'    => $slug,
...
<title><?= $action==='list' ? 'Manage Tours' : ($action==='edit' ? 'Edit Tour' : 'Add New Tour') ?> | Good Shepherd Tours & Travels Admin</title>
                    ...
                    if (strpos($rawItin, '<') !== false) {
                        if (preg_match('/<h[2-4]/i', $rawItin)) {
                            ...
                            foreach ($pm[1] as $pi => $ptitle) {
                                ...
                                if ($ptitle) $itinRows[] = ['title' => $ptitle, 'body' => trim(str_replace(["\r\n", "\r"], "\n", strip_tags($pm[2][$pi])))];
                            }
                        }
                        ...
                        foreach ($days as $pd) {
                            ...
                            $itinRows[] = ['title' => $pt, 'body' => implode("\n", $lines)];
                        }
                    }
                }
            }
              ...
              <div id="itinBuilder" style="display:flex;flex-direction:column;gap:10px;margin-top:6px">
                ...
                <div class="itin-day-row" style="border:1.5px solid var(--border);border-radius:12px;overflow:hidden;background:#fff;">
                  ...
                  <div style="padding:14px 16px">
                    ...
                    <div class="itin-media-fields frow c2" style="margin-top:14px">
                      <div class="fgrp">
                        <label style="font-size:11px">Day Image 1</label>
                        <input type="hidden" name="itin_existing_image1[]" class="itin-existing-image1" value="<?= htmlspecialchars($rday['image1'] ?? '') ?>"/>
                        <input type="file" name="itin_image1[]" class="form-control" accept="image/*"
                               onchange="previewItinImage(this, '.itin-preview-1', '.itin-wrap-1')"/>
                        <div class="itin-wrap-1" style="<?= empty($rday['image1']) ? 'display:none;margin-top:10px' : 'display:block;margin-top:10px' ?>">
                          <img src="<?= !empty($rday['image1']) ? SITE_URL . '/' . htmlspecialchars($rday['image1']) : '' ?>" class="img-preview itin-preview-1" alt=""/>
                        </div>
                      </div>
                      <div class="fgrp">
                        <label style="font-size:11px">Day Image 2</label>
                        <input type="hidden" name="itin_existing_image2[]" class="itin-existing-image2" value="<?= htmlspecialchars($rday['image2'] ?? '') ?>"/>
                        <input type="file" name="itin_image2[]" class="form-control" accept="image/*"
                               onchange="previewItinImage(this, '.itin-preview-2', '.itin-wrap-2')"/>
                        <div class="itin-wrap-2" style="<?= empty($rday['image2']) ? 'display:none;margin-top:10px' : 'display:block;margin-top:10px' ?>">
                          <img src="<?= !empty($rday['image2']) ? SITE_URL . '/' . htmlspecialchars($rday['image2']) : '' ?>" class="img-preview itin-preview-2" alt=""/>
                        </div>
                      </div>
                    </div>
                  </div>
...
const itineraryMediaState = <?= json_encode(array_values($itinRows ?? []), JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) ?>;

function mountItinMediaFields(row, data = {}) {
  if (!row || row.querySelector('.itin-media-fields')) return;

  const image1 = data.image1 || '';
  const image2 = data.image2 || '';
  const wrap = document.createElement('div');
  wrap.className = 'itin-media-fields frow c2';
  wrap.style.marginTop = '14px';
  wrap.innerHTML = `
    <div class="fgrp">
      <label style="font-size:11px">Day Image 1</label>
      <input type="hidden" name="itin_existing_image1[]" class="itin-existing-image1" value="${image1}">
      <input type="file" name="itin_image1[]" class="form-control" accept="image/*" onchange="previewItinImage(this, '.itin-preview-1', '.itin-wrap-1')">
      <div class="itin-wrap-1" style="${image1 ? 'display:block' : 'display:none'};margin-top:10px">
        <img src="${image1 ? '<?= rtrim(SITE_URL, '/') ?>/' + image1 : ''}" class="img-preview itin-preview-1" alt="">
      </div>
    </div>
    <div class="fgrp">
      <label style="font-size:11px">Day Image 2</label>
      <input type="hidden" name="itin_existing_image2[]" class="itin-existing-image2" value="${image2}">
      <input type="file" name="itin_image2[]" class="form-control" accept="image/*" onchange="previewItinImage(this, '.itin-preview-2', '.itin-wrap-2')">
      <div class="itin-wrap-2" style="${image2 ? 'display:block' : 'display:none'};margin-top:10px">
        <img src="${image2 ? '<?= rtrim(SITE_URL, '/') ?>/' + image2 : ''}" class="img-preview itin-preview-2" alt="">
      </div>
    </div>`;

  row.querySelector('.itin-body')?.insertAdjacentElement('afterend', wrap);
}
function addItinDay() {
  ...
  mountItinMediaFields(rows[rows.length - 1]);
...
function serializeItinerary() {
  ...
  document.querySelectorAll('#itinBuilder .itin-day-row').forEach(row => {
    ...
    const image1 = row.querySelector('.itin-existing-image1')?.value.trim() || '';
    const image2 = row.querySelector('.itin-existing-image2')?.value.trim() || '';
    if (title) {
      const day = { title, body };
      if (image1) day.image1 = image1;
      if (image2) day.image2 = image2;
      days.push(day);
    }
  });
...
function previewItinImage(input, imageSelector, wrapSelector) {
  const row = input.closest('.itin-day-row');
  if (!row || !input.files || !input.files[0]) return;
  const img = row.querySelector(imageSelector);
  const wrap = row.querySelector(wrapSelector);
  if (!img || !wrap) return;
...
document.querySelectorAll('#itinBuilder .itin-day-row').forEach((row, i) => {
  mountItinMediaFields(row, itineraryMediaState[i] || {});
});
</script>
</body>
</html>
