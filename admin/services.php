...
foreach ([
    ['label', 'VARCHAR(80) DEFAULT NULL', 'title'],
    ['link_url', "VARCHAR(300) DEFAULT 'tours.php'", 'icon'],
    ['link_text', "VARCHAR(80) DEFAULT 'Explore'", 'link_url'],
] as $migration) {
    if (!adminEnsureColumn($conn, 'services', $migration[0], $migration[1], $migration[2])) {
        $formError = 'Services schema update failed: ' . $conn->error;
        break;
    }
}
    ...
    if (!$title) {
        ...
        if (!$formError) {
            ...
            if ($postId) {
                ...
                $ok = $conn->query("UPDATE services SET
                    ...
                    WHERE id=$postId");
                if ($ok) { header('Location: services.php?msg=updated'); exit; }
                $formError = 'Unable to update service: ' . $conn->error;
                adminLogError('admin_services_update', ['mysql' => $conn->error, 'id' => $postId, 'title' => $title]);
            } else {
                $imgVal = $imagePath ? "'" . $conn->real_escape_string($imagePath) . "'" : 'NULL';
                $ok = $conn->query("INSERT INTO services (title, label, description, image, icon, link_url, link_text, sort_order, is_active)
                    VALUES ('$titleE','$lblE','$descE',$imgVal,'$iconE','$lurlE','$ltxtE',$sort_order,$is_active)");
                if ($ok) { header('Location: services.php?msg=added'); exit; }
                $formError = 'Unable to save service: ' . $conn->error;
                adminLogError('admin_services_insert', ['mysql' => $conn->error, 'title' => $title]);
            }
        }
    }
}
...
<title>What We Offer | Good Shepherd Tours & Travels Admin</title>
