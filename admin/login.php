...
try {
    $settingsRes = $conn->query("SELECT skey, sval FROM settings WHERE skey IN ('turnstile_site', 'turnstile_secret', 'turnstile_site_key', 'turnstile_secret_key')");
    $settingsMap = [];
    if ($settingsRes) {
        while ($row = $settingsRes->fetch_assoc()) {
            $settingsMap[$row['skey']] = $row['sval'];
        }
    }
    $turnstileSiteKey = trim($settingsMap['turnstile_site'] ?? $settingsMap['turnstile_site_key'] ?? '');
    $turnstileSecretKey = trim($settingsMap['turnstile_secret'] ?? $settingsMap['turnstile_secret_key'] ?? '');
    ...
    if (!$username || !$password) {
        ...
        if ($turnstileResponse === '') {
            ...
            $verify = @file_get_contents('https://challenges.cloudflare.com/turnstile/v0/siteverify', false,
                ...
                'content' => http_build_query(['secret' => $turnstileSecretKey, 'response' => $turnstileResponse])]]));
