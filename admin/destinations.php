...
/* ── SAFE MIGRATION ── */
foreach ([
    ['category_id', 'INT DEFAULT NULL', 'id'],
    ['content', 'LONGTEXT DEFAULT NULL', 'excerpt'],
    ['quick_facts', 'LONGTEXT DEFAULT NULL', 'content'],
    ['best_months', 'LONGTEXT DEFAULT NULL', 'quick_facts'],
    ['gallery', 'LONGTEXT DEFAULT NULL', 'best_months'],
    ['show_best_time', 'TINYINT(1) DEFAULT 1', 'best_months'],
] as $migration) {
    if (!adminEnsureColumn($conn, 'destinations', $migration[0], $migration[1], $migration[2])) {
        $errors[] = 'Destination schema update failed: ' . $conn->error;
        break;
    }
}
...
if ($action === 'delete' && isset($_GET['id'])) {
    $id  = (int)$_GET['id'];
    $row = $conn->query("SELECT hero_image, gallery FROM destinations WHERE id=$id")->fetch_assoc();
    if ($row) {
        $deletePaths = [];
        if (!empty($row['hero_image']) && strpos($row['hero_image'], 'uploads/') === 0) {
            $deletePaths[] = $row['hero_image'];
        }
        $galleryItems = !empty($row['gallery']) ? (json_decode($row['gallery'], true) ?: []) : [];
        foreach ($galleryItems as $galleryPath) {
            if ($galleryPath && strpos($galleryPath, 'uploads/') === 0) {
                $deletePaths[] = $galleryPath;
            }
        }
        foreach (array_unique($deletePaths) as $relPath) {
            $imgPath = __DIR__ . '/../' . $relPath;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $show_best_time = isset($_POST['show_best_time']) ? 1 : 0;
    ...
    $existingGallery = [];
    if ($destId) {
        $gr = $conn->query("SELECT gallery FROM destinations WHERE id=$destId LIMIT 1");
        $existingRow = $gr ? $gr->fetch_assoc() : null;
        $existingGallery = !empty($existingRow['gallery']) ? (json_decode($existingRow['gallery'], true) ?: []) : [];
    }
    $removedGallery = array_values(array_filter(array_map('trim', $_POST['remove_gallery'] ?? [])));
    if ($removedGallery) {
        $existingGallery = array_values(array_filter($existingGallery, function ($path) use ($removedGallery) {
            return !in_array($path, $removedGallery, true);
        }));
    }
    ...
    if (!empty($_FILES['gallery_images']['name']) && is_array($_FILES['gallery_images']['name'])) {
        foreach ($_FILES['gallery_images']['name'] as $i => $fileName) {
            ...
            $up = uploadImage($file, 'uploads/destinations/gallery');
            if (!$up['ok']) {
                $errors[] = $file['name'] . ': ' . $up['msg'];
                continue;
            }
            $galleryPaths[] = $up['path'];
        }
    }
    $galleryPaths = array_values(array_unique(array_filter($galleryPaths)));
    $galleryJson = json_encode($galleryPaths, JSON_UNESCAPED_UNICODE);
    ...
    if (!$errors) {
        ...
        $galleryE = $conn->real_escape_string($galleryJson);
        $catIdVal = $categoryId ?: 'NULL';

        foreach ($removedGallery as $removedPath) {
            if (strpos($removedPath, 'uploads/') === 0) {
                $absRemoved = __DIR__ . '/../' . $removedPath;
                if (file_exists($absRemoved)) unlink($absRemoved);
            }
        }

        if ($destId) {
            $imgClause = $imagePath ? ", hero_image='" . $conn->real_escape_string($imagePath) . "'" : '';
            $ok = $conn->query("UPDATE destinations SET
                title='$titleE', slug='$slugE', category_id=$catIdVal, location='$locE',
                region='$regE', read_time=$read_time, excerpt='$excE', content='$contE',
                quick_facts='$qfE', best_months='$bmE', gallery='$galleryE', show_best_time=$show_best_time,
                is_featured=$is_feat, is_active=$is_active, sort_order=$sort_order
                $imgClause
                WHERE id=$destId");
            if ($ok) { header('Location: destinations.php?msg=updated'); exit; }
            $errors[] = 'Unable to update destination: ' . $conn->error;
            adminLogError('admin_destinations_update', ['mysql' => $conn->error, 'id' => $destId, 'title' => $title]);
        } else {
            $imgVal = $imagePath ? "'" . $conn->real_escape_string($imagePath) . "'" : "''";
            $ok = $conn->query("INSERT INTO destinations
                (title, slug, category_id, location, region, hero_image, read_time,
                 excerpt, content, quick_facts, best_months, gallery, show_best_time, is_featured, is_active, sort_order)
                VALUES
                ('$titleE','$slugE',$catIdVal,'$locE','$regE',$imgVal,$read_time,
                 '$excE','$contE','$qfE','$bmE','$galleryE',$show_best_time,$is_feat,$is_active,$sort_order)");
            if ($ok) { header('Location: destinations.php?msg=added'); exit; }
            $errors[] = 'Unable to save destination: ' . $conn->error;
            adminLogError('admin_destinations_insert', ['mysql' => $conn->error, 'title' => $title]);
        }
    }
...
$vGallery   = !empty($v['gallery']) ? (json_decode($v['gallery'], true) ?: []) : [];
?>
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8"/><meta name="viewport" content="width=device-width,initial-scale=1.0"/>
<title>Destinations | Good Shepherd Tours & Travels Admin</title>
...
.gallery-upload-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}
.gallery-upload-item{position:relative;background:#fff;border:1px solid var(--border);border-radius:10px;padding:8px}
.gallery-upload-item img{width:100%;height:110px;object-fit:cover;border-radius:8px;border:1px solid var(--border);display:block;background:var(--off-white)}
.gallery-upload-item code{display:block;font-size:10px;color:var(--text-light);margin-top:4px;word-break:break-all}
.gallery-remove-check{display:flex;align-items:center;gap:8px;font-size:12px;color:var(--red);font-weight:600;margin-top:8px;cursor:pointer}
.gallery-remove-check input[type=checkbox]{accent-color:var(--red)}
      ...
      <form method="POST" enctype="multipart/form-data" id="destForm"
        ...
        <div class="dest-form-grid">
            ...
            <div class="form-section">
              ...
              <div class="form-sec-body">
                <label class="check-row" style="margin-bottom:12px">
                  <input type="checkbox" name="show_best_time" value="1" <?= !isset($v['show_best_time']) || $v['show_best_time'] ? 'checked' : '' ?>/>
          ...
          <div style="display:flex;flex-direction:column;gap:16px">
            ...
            <div class="form-section">
              <div class="form-sec-head"><i class="fas fa-images"></i><h3>Destination Gallery</h3></div>
              <div class="form-sec-body">
                <input type="file" name="gallery_images[]" accept="image/*" multiple class="form-control" style="font-size:13px"/>
                <p style="font-size:11px;color:var(--text-light);margin:4px 0 0">Upload multiple gallery images for this destination. When editing, new uploads are added to the existing gallery.</p>
                <?php if ($vGallery): ?>
                  <div class="gallery-upload-grid">
                    <?php foreach ($vGallery as $galleryPath): ?>
                      <?php
                        $galleryUrl = strpos($galleryPath, 'uploads/') === 0
                            ? SITE_URL . '/' . $galleryPath
                            : SITE_URL . '/images/' . $galleryPath;
                      ?>
                      <div class="gallery-upload-item">
                        <img src="<?= htmlspecialchars($galleryUrl) ?>" alt="Destination gallery image"/>
                        <code><?= htmlspecialchars($galleryPath) ?></code>
                        <label class="gallery-remove-check">
                          <input type="checkbox" name="remove_gallery[]" value="<?= htmlspecialchars($galleryPath) ?>"/>
