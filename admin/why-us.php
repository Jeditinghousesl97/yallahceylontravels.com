...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$title) {
        ...
        if ($postId) {
            $ok = $conn->query("UPDATE why_us SET
                title='$titleE', description='$descE', icon='$iconE',
                sort_order=$sort_order, is_active=$is_active
                WHERE id=$postId");
            if ($ok) { header('Location: why-us.php?msg=updated'); exit; }
            $formError = 'Unable to update item: ' . $conn->error;
            adminLogError('admin_why_us_update', ['mysql' => $conn->error, 'id' => $postId, 'title' => $title]);
        } else {
            $ok = $conn->query("INSERT INTO why_us (title, description, icon, sort_order, is_active)
                VALUES ('$titleE','$descE','$iconE',$sort_order,$is_active)");
            if ($ok) { header('Location: why-us.php?msg=added'); exit; }
            $formError = 'Unable to save item: ' . $conn->error;
            adminLogError('admin_why_us_insert', ['mysql' => $conn->error, 'title' => $title]);
        }
    }
}
...
<title>Why Choose Us | Good Shepherd Tours & Travels Admin</title>
