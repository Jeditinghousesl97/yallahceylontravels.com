...
if (!adminEnsureColumn($conn, 'blog_posts', 'meta', 'LONGTEXT DEFAULT NULL', 'is_published')) {
    $errors[] = 'Blog schema update failed: ' . $conn->error;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        ...
        if ($postId) {
            ...
            $ok = $conn->query("UPDATE blog_posts SET
                ...
                WHERE id=$postId");
            if ($ok) { header('Location: blog.php?msg=updated'); exit; }
            $errors[] = 'Unable to update post: ' . $conn->error;
            adminLogError('admin_blog_update', ['mysql' => $conn->error, 'id' => $postId, 'title' => $title]);
        } else {
            $imgVal   = $imagePath ? "'" . $conn->real_escape_string($imagePath) . "'" : 'NULL';
            $pubAtSQL = $is_pub ? "NOW()" : 'NULL';
            $ok = $conn->query("INSERT INTO blog_posts
            ...
            if ($ok) { header('Location: blog.php?msg=added'); exit; }
            $errors[] = 'Unable to save post: ' . $conn->error;
            adminLogError('admin_blog_insert', ['mysql' => $conn->error, 'title' => $title]);
        }
    }
