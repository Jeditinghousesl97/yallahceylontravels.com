...
foreach ([
    ['facebook_url', 'VARCHAR(300) DEFAULT NULL', 'photo'],
    ['instagram_url', 'VARCHAR(300) DEFAULT NULL', 'facebook_url'],
    ['whatsapp', 'VARCHAR(100) DEFAULT NULL', 'instagram_url'],
] as $migration) {
    if (!adminEnsureColumn($conn, 'team', $migration[0], $migration[1], $migration[2])) {
        $errors[] = 'Team schema update failed: ' . $conn->error;
        break;
    }
}
    ...
    if (!$errors) {
        ...
        if ($postId) {
            ...
            $ok = $conn->query("UPDATE team SET
                ...
                WHERE id=$postId");
            if ($ok) { header('Location: team.php?msg=updated'); exit; }
            $errors[] = 'Unable to update team member: ' . $conn->error;
            adminLogError('admin_team_update', ['mysql' => $conn->error, 'id' => $postId, 'name' => $name]);
        } else {
            $imgVal = $photoPath ? "'" . $conn->real_escape_string($photoPath) . "'" : 'NULL';
            $ok = $conn->query("INSERT INTO team
            ...
            if ($ok) { header('Location: team.php?msg=added'); exit; }
            $errors[] = 'Unable to save team member: ' . $conn->error;
            adminLogError('admin_team_insert', ['mysql' => $conn->error, 'name' => $name]);
        }
    }
...
<title>Team | Good Shepherd Tours & Travels Admin</title>
            ...
            <div class="form-section">
              ...
              <div class="form-sec-body">
                ...
                <div class="fgrp">
                  ...
                  <input type="text" name="whatsapp" class="form-control" placeholder="94775775211 (no + or spaces)" value="<?= htmlspecialchars($v['whatsapp'] ?? '') ?>"/>
                  <small style="color:var(--text-light);font-size:11px">Country code + number, no spaces or +. e.g. 94775775211</small>
                </div>
              </div>
            </div>
